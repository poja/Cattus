...
struct HexState {
    ...
    board: [[Hexagon; GAME_SIZE]; GAME_SIZE],
    turn: Color,
    is_over: bool,
    winner: Option<Color>,
}
...
impl HexState {
    ...
    fn make_turn(&mut self, loc: Location) -> bool {
        if self.is_over {
            return false;
        }
        if !in_board(loc) {
...
fn location_neighbors(loc: Location) -> Vec<Location> {
    ...
    if loc.0 > 0 {
        candidates.insert(0, (loc.0 - 1, loc.1));
    }
    if loc.1 > 0 {
        candidates.insert(0, (loc.0, loc.1 - 1));
    }
    if loc.0 > 0 && loc.1 > 0 {
        candidates.insert(0, (loc.0 - 1, loc.1 - 1));
    }
    candidates
        .into_iter()
...
fn make_random_turn(game: &mut HexState) {
